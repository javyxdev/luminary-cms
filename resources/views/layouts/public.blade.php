...
<body class="lmn-bg-dark text-white antialiased" style="font-family: 'Inter', sans-serif;">
    ...
    <nav class="fixed top-0 left-0 right-0 z-50 lmn-bg-dark border-b border-white/10" x-data="{ menuOpen: false }">
        ...
        {{-- Fila superior: Social | Logo | Hamburger (grid 3 cols garantiza centrado) --}}
        <div class="max-w-7xl mx-auto px-5 pt-3" style="display: grid; grid-template-columns: 1fr auto 1fr; align-items: center;">
            ...
            {{-- Col 2: Logo (auto width, siempre centrado) --}}
            <a href="{{ route('home') }}">
                @if($logoUrl)
                    <img src="{{ $logoUrl }}" alt="Luminary" style="height: 51px; width: auto; display: block;">
                @else
                    <span class="font-display text-4xl tracking-[0.3em] text-white font-bold">LUMINARY</span>
                @endif
            </a>

            {{-- Col 3: Hamburger móvil / vacío en desktop --}}
            <div style="display: flex; justify-content: flex-end; align-items: center;">
                <button @click="menuOpen = !menuOpen"
                        class="md:hidden text-white/70 hover:text-white transition-colors" aria-label="Menú">
